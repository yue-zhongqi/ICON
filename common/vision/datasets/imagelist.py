...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str,
        ...
        self.metadata = [{"index": i} for i in range(len(self.samples))]
    ...
    def set_metadata(self, metadata, name):
        # assert isinstance(metadata, list)
        assert len(metadata) == len(self.samples)
        for i in range(len(metadata)):
            self.metadata[i][name] = metadata[i]
